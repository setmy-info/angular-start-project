...
function logAppStarted(): void {
    ...
    log.info(
        ...
        version,
        ', for: ',
        angularStartProjectLibrary.sessionService.getSessionId(),
    );
    if (versionState.isNewVersion) {
        log.info(
            ...
            ' (previous: ',
            versionState.previousVersion ?? 'none — first visit',
            ')',
        );
    }
}
...
function logAppOpenedLocationOnce(): void {
    ...
    if (typeof window !== 'undefined' && window.isSecureContext === false) {
        const message =
            'Geolocation requires HTTPS (or localhost) — this page was opened over an insecure origin.';
        log.warn(message);
        locationService.setError(message);
        return;
    }
