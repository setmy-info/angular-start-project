...
describe('LanguageService', () => {
    ...
    it('fetches and caches translations when nothing is cached yet', async () => {
        const fetchMock = vi.fn((url: string) => Promise.resolve(url.includes('translations-version.json')
        ...
        expect(JSON.parse(localStorage.getItem('translations.et') as string)['app.title']).toBe('Fetched Title');
    ...
    it('loads the newly selected language on changeLanguage', async () => {
        const fetchMock = vi.fn((url: string) => Promise.resolve(url.includes('translations-version.json')
            ? jsonResponse({et: 1, en: 1})
            : jsonResponse(url.includes('/en.json') ? {'app.title': 'English Title'} : {'app.title': 'Estonian Title'})));
        vi.stubGlobal('fetch', fetchMock);
