...
//
// A step is an async (or plain) function, run in list order. In this workspace
// the integration tier reads built artifacts from disk and the e2e tier drives
// Selenium against the built Angular app, so the static servers started here
// are the lifecycle slot (manual `npm run server` never collides: test ports
// are config.server.port + 1, and never 4200 where `ng serve` listens). A
// project using this repo as its template adds what its own tiers need next
// to them. Post steps must stay idempotent - CI runs them again after a failed
