...
const stateDirectory = path.join(rootDir, 'build', 'servers');
...
async function startServer(parsedArgs) {
    ...
    if (fs.existsSync(stateFile)) {
        ...
        if (isProcessAlive(previous.pid)) {
            console.error(
                `HTTP server for port ${port} is already registered (pid ${previous.pid}). Stop it first: node scripts/http-server.js stop --port ${port}`,
            );
            process.exit(1);
        }
...
// Stops every server registered under build/servers/ - used by
...
async function serve(parsedArgs) {
    ...
    const server = http.createServer((request, response) => {
        ...
        try {
            ...
            // SPA fallback: a client-routed path (/about) is not a file on
            // disk, so serve index.html and let the app router resolve it -
            // what a real static host does for an Angular application. Opt-in
            // via package.json config.server.spa, so a plain asset server
            // still 404s honestly.
            const indexPath = path.join(directory, 'index.html');

            if (spaFallback && !path.extname(filePath) && fs.existsSync(indexPath)) {
                response.writeHead(200, {
                    'Content-Type': 'text/html; charset=utf-8',
                });
