...
try {
    ...
    output = execSync(`${npmCommand} ls --all --json --workspace "${workspace.packageName}"`, {
        cwd: rootDir,
        encoding: 'utf8',
    });
} catch (error) {
...
fs.writeFileSync(path.join(outDir, 'dependency-tree.json'), `${JSON.stringify(tree, null, 2)}\n`);
...
writePage(path.join(outDir, 'index.html'), `Dependency tree - ${workspace.packageName}`, body);
